...
public class GuiOredictionificatorFilter extends GuiTextFilter<OredictionificatorFilter, TileEntityOredictionificator> {
    ...
    protected void init() {
        super.init();
        addChild(new MekanismImageButton(guiObj, field_230690_l_ + 10, field_230691_m_ + 18, 12, getButtonLocation("left"), () -> {
            if (filter.hasFilter()) {
                filter.previous();
                slotDisplay.updateStackList();
            }
        }, getOnHover(MekanismLang.LAST_ITEM)));
        addChild(new MekanismImageButton(guiObj, field_230690_l_ + 10, field_230691_m_ + 52, 12, getButtonLocation("right"), () -> {
            if (filter.hasFilter()) {
                filter.next();
                slotDisplay.updateStackList();
            }
    ...
    protected void setText() {
        ...
        if (filter.hasFilter() && filter.filterMatches(filterLocation)) {
            ...
            if (possibleFilters.stream().anyMatch(newFilter::startsWith) &&
                TagCollectionManager.func_232928_e_().func_232925_b_().getRegisteredTags().contains(filterLocation)) {
