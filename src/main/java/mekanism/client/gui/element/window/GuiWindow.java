...
public class GuiWindow extends GuiElement implements IGUIWindow {
    ...
    public void renderBackgroundOverlay(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        if (isFocusOverlay()) {
            //TODO - 26.3: This used to pass rgba instead of argb, which is wrong. See if the color overlay still renders as expected, or if we wanted the messed up values
            MekanismRenderer.renderColorOverlay(guiGraphics, -getGuiLeft(), -getGuiTop(), OVERLAY_COLOR.argb());
        } else {
            //TODO - 26.3: check this vs the old. Looks rather strong on top of other windows
