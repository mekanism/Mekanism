...
public class GuiDynamicHorizontalRateBar extends GuiBar<IBarInfoHandler> {
    ...
    protected void renderBarOverlay(MatrixStack matrix, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i < displayInt; i++) {
            ...
            if (i == 0) {
                func_238463_a_(matrix, field_230690_l_ + 1, field_230691_m_ + 1, 0, 0, 1, texHeight, texWidth, texHeight);
            } else if (i == displayInt - 1) {
                func_238463_a_(matrix, field_230690_l_ + 1 + i, field_230691_m_ + 1, texWidth - 1, 0, 1, texHeight, texWidth, texHeight);
            } else {
                func_238463_a_(matrix, field_230690_l_ + 1 + i, field_230691_m_ + 1, 1, 0, 1, texHeight, texWidth, texHeight);
            }
