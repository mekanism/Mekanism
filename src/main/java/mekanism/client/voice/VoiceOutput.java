            ...
            while (voiceClient.running) {
                try {
                    if (voiceClient.input.available() > 0) {
                        byteCount = voiceClient.input
                              .readUnsignedShort(); //Why would we only read signed shorts? negative amount of waiting data doesn't make sense anyway :D
                        while (byteCount > 0 && voiceClient.running) {
                            length = audioData.length;
                            if (length > byteCount) {
                                length = byteCount;
                            }
                            length = voiceClient.input.read(audioData, 0,
                                  length); // That one returns the actual read amount of data (we can begin transferring even if input is waiting/incomplete)
                            if (length < 0) {
