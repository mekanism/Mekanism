...
public class TileSound extends MekSound implements IResettableSound {
    ...
    public TileSound(IHasSound source, ISoundSource values) {
        this(source, values.getSoundLocation(), values.getVolume(), values.getFrequency(), values.shouldRepeat(),
              values.getRepeatDelay(), values.getSoundPosition().x, values.getSoundPosition().y,
              values.getSoundPosition().z);
    }

    public TileSound(IHasSound source, ISoundSource values, ResourceLocation location) {
        this(source, location, values.getVolume(), values.getFrequency(), values.shouldRepeat(),
              values.getRepeatDelay(), values.getSoundPosition().x, values.getSoundPosition().y,
              values.getSoundPosition().z);
    }

    public TileSound(IHasSound source, String sound, float volume, float pitch, boolean repeat, int repeatDelay,
    ...
    public TileSound(IHasSound source, ResourceLocation sound, float volume, float pitch, boolean repeat,
          int repeatDelay, double x, double y, double z) {
        this(source, sound, volume, pitch, repeat, repeatDelay, x, y, z, AttenuationType.LINEAR);
    }

    public TileSound(IHasSound soundSource, ResourceLocation resource, float volume, float pitch, boolean repeat,
