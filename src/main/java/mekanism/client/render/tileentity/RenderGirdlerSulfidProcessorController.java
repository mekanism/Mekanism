...
public class RenderGirdlerSulfidProcessorController extends TileEntitySpecialRenderer
{
	private static Map<GirdlerRenderData, HashMap<Fluid, DisplayInteger[]>> cachedCenterFluids = new HashMap<GirdlerRenderData, HashMap<Fluid, DisplayInteger[]>>();

	@Override
	public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float partialTick)
	{
		renderAModelAt((TileEntityGirdlerSulfidProcessorController)tileEntity, x, y, z, partialTick);
	}

	public void renderAModelAt(TileEntityGirdlerSulfidProcessorController tileEntity, double x, double y, double z, float partialTick)
	{
		if(tileEntity.structured && tileEntity.inputTank.getFluid() != null && tileEntity.inputTankSnd.getFluid() != null)
		{
			GirdlerRenderData data = new GirdlerRenderData();
			...
			if(data.height >= 1 && tileEntity.inputTank.getCapacity() > 0 && tileEntity.inputTankSnd.getCapacity() > 0)
			{
				...
				MekanismRenderer.glowOn(tileEntity.inputTankSnd.getFluid().getFluid().getLuminosity());

				DisplayInteger[] displayList = getListAndRender(data, tileEntity.inputTank.getFluid().getFluid());
                DisplayInteger[] displayListSnd = getListAndRender(data, tileEntity.inputTankSnd.getFluid().getFluid());
                ...
                displayListSnd[(int)(((float)tileEntity.inputTankSnd.getFluidAmount()/tileEntity.inputTankSnd.getCapacity())*((float)getStages(data.height)-1))].render();
	...
	private DisplayInteger[] getListAndRender(GirdlerRenderData data, Fluid fluid)
	{
	...
	public static class GirdlerRenderData
	{
		public int height;
		...
		{
			return data instanceof GirdlerRenderData && ((GirdlerRenderData)data).height == height &&
					((GirdlerRenderData)data).side == side;
		}
	}
