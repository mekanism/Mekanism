...
public class RenderSPS extends MekanismTileEntityRenderer<TileEntitySPSCasing> {
    ...
    @Override
    public boolean shouldRender(TileEntitySPSCasing tile, Vec3 camera) {
        if (tile.isMaster()) {
            SPSMultiblockData multiblock = tile.getMultiblock();
            return multiblock.isFormed() && multiblock.renderLocation != null && super.shouldRender(tile, camera);
        }
        return false;
    }
}
