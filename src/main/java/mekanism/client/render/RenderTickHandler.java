	...
	public void tickEnd(RenderTickEvent event)
	{
		if(event.phase == Phase.END)
		{
			if(mc.player != null && mc.world != null && !mc.isGamePaused())
			{
				...
				if(player != null && mc.currentScreen == null && player.getItemStackFromSlot(EntityEquipmentSlot.CHEST) != ItemStack.EMPTY)
				{
				...
				if(world.getWorldTime() % 4 == 0)
				{
					...
					{
						if(!Mekanism.flamethrowerActive.contains(p.getName()) && !p.isSwingInProgress && p.inventory.getCurrentItem() != ItemStack.EMPTY && p.inventory.getCurrentItem().getItem() instanceof ItemFlamethrower)
						{
