    ...
    public GuiWindGenerator(InventoryPlayer inventory, TileEntityWindGenerator tentity) {
        ...
        guiElements.add(new GuiEnergyInfo(() -> ListUtils.asList(
              LangUtils.localize("gui.producing") + ": " + MekanismUtils.getEnergyDisplay(
                    tileEntity.isActive ? MekanismConfig.current().generators.windGenerationMin.val()
                          * tileEntity.currentMultiplier : 0) + "/t",
        ...
        guiElements
              .add(new GuiPowerBar(this, tileEntity, MekanismUtils.getResource(ResourceType.GUI, "GuiWindTurbine.png"),
                    164, 15));
        guiElements
              .add(new GuiSlot(SlotType.NORMAL, this, MekanismUtils.getResource(ResourceType.GUI, "GuiWindTurbine.png"),
                    142, 34).with(SlotOverlay.POWER));
    }
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        fontRenderer
              .drawString(MekanismUtils.getEnergyDisplay(tileEntity.getEnergy(), tileEntity.getMaxEnergy()), 51, 26,
                    0x00CD00);
        fontRenderer.drawString(LangUtils.localize("gui.power") + ": " + powerFormat.format(MekanismUtils
                    .convertToDisplay(
                          MekanismConfig.current().generators.windGenerationMin.val() * tileEntity.currentMultiplier)), 51,
              35, 0x00CD00);
        ...
        if (!tileEntity.getActive()) {
            size += 9;
            final String message = LangUtils
                  .localize(tileEntity.isInBlacklistedDimension() ? "mekanism.gui.noWind" : "gui.skyBlocked");
