...
public class GuiWindGenerator extends GuiMekanism
{
	...
	public GuiWindGenerator(InventoryPlayer inventory, TileEntityWindGenerator tentity)
	{
		...
		guiElements.add(new GuiEnergyInfo(() -> ListUtils.asList(
                LangUtils.localize("gui.producing") + ": " + MekanismUtils.getEnergyDisplay(tileEntity.isActive ? MekanismConfig.current().generators.windGenerationMin.val()*tileEntity.currentMultiplier : 0) + "/t",
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY)
	{
		...
		fontRenderer.drawString(LangUtils.localize("gui.power") + ": " + powerFormat.format(MekanismUtils.convertToDisplay(MekanismConfig.current().generators.windGenerationMin.val()*tileEntity.currentMultiplier)), 51, 35, 0x00CD00);
