...
public class PacketGeneratorsGui implements IMessageHandler<GeneratorsGuiMessage, IMessage>
{
	...
	public static class GeneratorsGuiMessage implements IMessage
	{
		...
		public static void openServerGui(int id, EntityPlayerMP playerMP, World world, Coord4D obj)
		{
			...
			playerMP.openContainer = MekanismGenerators.proxy.getServerGui(id, playerMP, world, obj.getPos().getX(), obj.getPos().getY(), obj.getPos().getZ());
		...
		{
			return (GuiScreen)MekanismGenerators.proxy.getClientGui(id, player, world, obj.getPos().getX(), obj.getPos().getY(), obj.getPos().getZ());
		}
