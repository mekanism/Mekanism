                    ...
                    for (z = zmin; z <= zmax; z++) {
                        if (x == xmin || x == xmax || y == ymin || y == ymax || z == zmin || z == zmax) {
                            if (!isViableNode(origX + x, origY + y, origZ + z)) {
                                rightBlocks = false;
                                break;
                            } else if (isFrame(coord.translate(x, y, z), origX + xmin, origX + xmax, origY + ymin,
                                  origY + ymax, origZ + zmin, origZ + zmax) && !isValidFrame(origX + x, origY + y,
                                  origZ + z)) {
                                rightFrame = false;
                                break;
                            } else {
                            ...
                            if (!isValidInnerNode(origX + x, origY + y, origZ + z)) {
                                ...
                                if (!isAir(origX + x, origY + y, origZ + z)) {
                                    innerNodes.add(new Coord4D(origX + x, origY + y, origZ + z,
        ...
        if (Math.abs(xmax - xmin) + 1 <= 18 && Math.abs(ymax - ymin) + 1 <= 18 && Math.abs(zmax - zmin) + 1 <= 18) {
            if (rightBlocks && rightFrame && isHollow && isCorner) {
                ...
                if (structure.volLength >= 3 && structure.volHeight >= 3 && structure.volWidth >= 3) {
                    ...
                    if (structure.locations.contains(Coord4D.get(pointer)) && isCorrectCorner(coord, origX + xmin,
                          origY + ymin, origZ + zmin)) {
