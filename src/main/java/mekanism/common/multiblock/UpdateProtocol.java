...
public abstract class UpdateProtocol<T extends SynchronizedData<T>> {
            ...
            if (isValid) {
                ...
                if (length <= 18 && height <= 18 && width <= 18) {
                    ...
                    if (structure.volLength >= 3 && structure.volHeight >= 3 && structure.volWidth >= 3) {
                        ...
                        if (structure.locations.contains(Coord4D.get(pointer)) && isCorrectCorner(coord, minX,
                              minY, minZ)) {
     ...
     * @param xShift Direction x is being changed, 1 is increasing, 0 means not changing, -1 means decreasing. Only one
     * of xShift, yShift, and zShift should not be 0 during any call. A value of 1 also implies that it is a viable node
     * so we start checking at 1 instead of 0.
     * @param yShift Direction y is being changed, 1 is increasing, 0 means not changing, -1 means decreasing. Only one
     * of xShift, yShift, and zShift should not be 0 during any call. A value of 1 also implies that it is a viable node
     * so we start checking at 1 instead of 0.
     * @param zShift Direction z is being changed, 1 is increasing, 0 means not changing, -1 means decreasing. Only one
     * of xShift, yShift, and zShift should not be 0 during any call. A value of 1 also implies that it is a viable node
     * so we start checking at 1 instead of 0.
    ...
    protected abstract void mergeCaches(List<ItemStack> rejectedItems, MultiblockCache<T> cache,
          MultiblockCache<T> merge);
    ...
    protected void onStructureCreated(T structure, int origX, int origY, int origZ, int xmin, int xmax, int ymin,
