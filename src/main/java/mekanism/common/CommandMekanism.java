	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] params)
	{
		...
		else if(params.length >= 1)
		{
			...
			else if(params[0].equalsIgnoreCase("teleporter"))
			{
				if(params.length == 2)
				{
					sender.sendMessage(new TextComponentString(EnumColor.DARK_BLUE + "[Mekanism]" + EnumColor.GREY + " Invalid parameters."));
				}
					...
					if(params[2].equalsIgnoreCase("list"))
					{
						...
						if(params.length == 3)
						{
							...
							if(manager != null)
							{
								...
								sender.sendMessage(new TextComponentString(EnumColor.DARK_BLUE + "[Mekanism]" + EnumColor.GREY + " User profile doesn't exist."));
							}
						}
					...
					else if(params[2].equalsIgnoreCase("delete"))
					{
						if(params.length == 4)
						{
							if(Mekanism.publicTeleporters.containsFrequency(params[3].trim()))
							{
								...
								sender.sendMessage(new TextComponentString(EnumColor.DARK_BLUE + "[Mekanism]" + EnumColor.GREY + " Successfully removed frequency."));
							}
							else {
								sender.sendMessage(new TextComponentString(EnumColor.DARK_BLUE + "[Mekanism]" + EnumColor.GREY + " No such frequency found."));
							}
						}
					...
					else if(params[2].equalsIgnoreCase("deleteAll"))
					{
						if(params.length == 4)
						{
							...
							if(manager != null)
							{
								...
								sender.sendMessage(new TextComponentString(EnumColor.DARK_BLUE + "[Mekanism]" + EnumColor.GREY + " Successfully removed " + amount + " frequencies."));
							}
							else {
								sender.sendMessage(new TextComponentString(EnumColor.DARK_BLUE + "[Mekanism]" + EnumColor.GREY + " User profile doesn't exist."));
							}
