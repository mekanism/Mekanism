...
public class ComponentBackedChemicalTank extends ComponentBackedContainer<ChemicalStack, AttachedChemicals> implements IChemicalTank {
    ...
    private final BiPredicate<ChemicalResource, AutomationType> canExtract;
    private final BiPredicate<ChemicalResource, AutomationType> canInsert;
    private final Predicate<ChemicalResource> validator;
    @Nullable
    ...
    public boolean isValid(ChemicalStack stack) {
        return getAttributeValidator().process(stack) && validator.test(ChemicalResource.of(stack));
    }
    ...
    public ChemicalStack insert(ChemicalStack stack, Action action, AutomationType automationType) {
        ...
        if (stack.isEmpty() || !isValid(stack) || !canInsert.test(ChemicalResource.of(stack), automationType)) {
    ...
    protected ChemicalStack extract(AttachedChemicals attachedChemicals, ChemicalStack stored, long amount, Action action, AutomationType automationType) {
        if (amount < 1 || stored.isEmpty() || !canExtract.test(ChemicalResource.of(stored), automationType)) {
