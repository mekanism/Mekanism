...
public abstract class TransitRequest implements Iterable<ItemData> {
    ...
    //TODO - 26.3: Evaluate what callers of this could just be replaced with using ResourceHandlerUtil calls
    public static TransitRequest definedItem(ResourceHandler<ItemResource> inventory, int min, int max, Finder finder, @Nullable TransactionContext transaction) {
        ...
        try (Transaction simulation = Transaction.open(transaction)) {
            ...
            for (int i = inventory.size() - 1; i >= 0; i--) {
                ...
                //TODO - 26.3: Do we want to try and rework the handler transit requests to be able to ignore index?
                //TODO - 26.3: Should we be limiting the passed max based on the max stack size? extract used to be limited to a stack at a time
