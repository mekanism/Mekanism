...
/// Basic implementation for [IInputRecipeCache] for handling recipes with three inputs.
public abstract class TripleInputRecipeCache<HOLDER_A, INPUT_A extends TypedInstance<HOLDER_A>, INGREDIENT_A extends InputIngredient<HOLDER_A, INPUT_A>, HOLDER_B, INPUT_B extends TypedInstance<HOLDER_B>, INGREDIENT_B extends InputIngredient<HOLDER_B, INPUT_B>,
    ...
    /// and inputB and inputC is empty without doing any extra validation on inputA. If however inputA is not empty and only one of the other two inputs is empty this
    /// will do validation on inputA and the non-empty input.
    ///
    /// @param world  World.
    ...
    ///
    /// @return `true` if there is a match or if inputA is not empty and inputB and inputC are both empty.
    ///
    /// @apiNote If you are trying to insert inputA call this method, otherwise call [#containsInputBAC] or [#containsInputCAB] depending on which input is trying to be
    /// inserted.
    ...
    /// Checks is there is a matching recipe with the given inputs. This method exists as a helper for insertion predicates and will return true if inputB is not empty
    /// and inputA and inputC is empty without doing any extra validation on inputB. If however inputB is not empty and only one of the other two inputs is empty this
    /// will do validation on inputB and the non-empty input.
    ///
    /// @param world  World.
    /// @param inputA Recipe input A.
    /// @param inputB Recipe input B.
    /// @param inputC Recipe input C.
    ///
    /// @return `true` if there is a match or if inputB is not empty and inputA and inputC are both empty.
    ///
    /// @apiNote If you are trying to insert inputB call this method, otherwise call [#containsInputABC] or [#containsInputCAB] depending on which input is trying to be
    /// inserted.
    ...
    /// Checks is there is a matching recipe with the given inputs. This method exists as a helper for insertion predicates and will return true if inputC is not empty
    /// and inputA and inputB is empty without doing any extra validation on inputC. If however inputC is not empty and only one of the other two inputs is empty this
    /// will do validation on inputC and the non-empty input.
    ///
    /// @param world  World.
    /// @param inputA Recipe input A.
    /// @param inputB Recipe input B.
    /// @param inputC Recipe input C.
    ///
    /// @return `true` if there is a match or if inputC is not empty and inputA and inputB are both empty.
    ///
    /// @apiNote If you are trying to insert inputC call this method, otherwise call [#containsInputABC] or [#containsInputBAC] depending on which input is trying to be
    /// inserted.
    ...
    /// - If only the first input is empty: This will check if input two and three are contained.
    /// - If only the second input is empty: This will check if input one and three are contained.
    /// - If only the third input is empty: This will check if input one and two are contained.
    /// - If only the first and third input is empty: This will check if the second input is contained.
    /// - If only the first input is not empty: This will return true.
