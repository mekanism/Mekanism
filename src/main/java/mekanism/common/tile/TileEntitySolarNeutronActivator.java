...
public class TileEntitySolarNeutronActivator extends TileEntityContainerBlock implements IRedstoneControl, IBoundingBlock, IGasHandler, ITubeConnection, IActiveState, ISustainedData, ITankManager, ISecurityTile, IUpgradeTile, IUpgradeInfoHandler
{
	...
	public void onUpdate() 
	{
		...
		if(!world.isRemote)
		{
			...
			if(inventory.get(0) != ItemStack.EMPTY && (inputTank.getGas() == null || inputTank.getStored() < inputTank.getMaxGas()))
			{
				inputTank.receive(GasTransmission.removeGas(inventory.get(0), inputTank.getGasType(), inputTank.getNeeded()), true);
			}
			
			if(inventory.get(1) != ItemStack.EMPTY && outputTank.getGas() != null)
			{
