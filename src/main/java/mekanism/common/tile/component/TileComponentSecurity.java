...
public class TileComponentSecurity implements ITileComponent {
    ...
    public void deserialize(ValueInput securityInput) {
        securityMode = securityInput.read(SerializationConstants.SECURITY_MODE, SecurityMode.CODEC).orElse(SecurityMode.PUBLIC);
    ...
    public void serialize(ValueOutput securityOutput) {
        if (securityMode != SecurityMode.PUBLIC) {
            securityOutput.store(SerializationConstants.SECURITY_MODE, SecurityMode.CODEC, securityMode);
        }
