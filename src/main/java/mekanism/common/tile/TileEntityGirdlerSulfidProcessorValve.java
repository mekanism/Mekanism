...
public class TileEntityGirdlerSulfidProcessorValve extends TileEntityGirdlerSulfidProcessorBlock implements IFluidHandler
{
		...
		if(!worldObj.isRemote)
		{
			if((master != null) != prevMaster)
			{
				for(ForgeDirection side : ForgeDirection.VALID_DIRECTIONS)
				{
					...
					if(!obj.isAirBlock(worldObj) && !(obj.getTileEntity(worldObj) instanceof TileEntityGirdlerSulfidProcessorBlock))
					{
	...
	{
		TileEntityGirdlerSulfidProcessorController controller = getController();
	...
	public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain)
	{
		...
		if(controller != null && (resource == null || resource.isFluidEqual(controller.outputTankSnd.getFluid())))
        {
            return controller.outputTankSnd.drain(resource.amount, doDrain);
        }
		return null;
	}
