...
import mekanism.common.recipe.machines.GirdlerSulfidProcessorRecipe;
...
public class TileEntityGirdlerSulfidProcessorController extends TileEntityGirdlerSulfidProcessorBlock implements IActiveState, ITankManager
{
	...
	public FluidTank inputTankSnd = new FluidTank(0);
	public FluidTank outputTank = new FluidTank(MAX_OUTPUT);
	public FluidTank outputTankSnd = new FluidTank(MAX_OUTPUT);
    ...
    public double partialInputSnd = 0;
	public double partialOutput = 0;
    public double partialOutputSnd = 0;
    ...
    public float prevScaleSnd;

	public TileEntityGirdlerSulfidProcessorController()
	{
		super("GirdlerSulfidProcessorController");

		/*
		 * Removed because there is no input/output for buckets
		 *
	...
	public void onUpdate()
	{
		...
		if(!worldObj.isRemote)
		{
			...
			updateTemperature();
			// manageBuckets(); // not used at this moment
			
			GirdlerSulfidProcessorRecipe recipe = getRecipe();
	
			if(canOperate(recipe))
			{
				int outputNeeded = outputTank.getCapacity()-outputTank.getFluidAmount();
                int outputNeededSnd = outputTankSnd.getCapacity()-outputTankSnd.getFluidAmount();
				int inputStored = inputTank.getFluidAmount();
                int inputStoredSnd = inputTankSnd.getFluidAmount();
				
				partialInput += Math.min(inputTank.getFluidAmount(), getTemperature()*recipe.recipeInput.ingredient.amount);
                partialInputSnd += Math.min(inputTankSnd.getFluidAmount(), getTemperature()*recipe.recipeInput.ingredient.amount);

				
				if(partialInput >= 1 && partialInputSnd >= 1)
				{
                    ...
                    int inputIntSnd = (int)Math.floor(partialInputSnd);
                    inputTankSnd.drain(inputInt, true);
                    partialInputSnd %= 1;
                    partialOutputSnd += ((double)inputIntSnd/recipe.recipeInput.ingredient.amount);
				}
				
				if(partialOutput >= 1 && partialOutputSnd >= 1)
				{
                    ...
                    int outputIntSnd = (int)Math.floor(partialOutputSnd);
                    outputTankSnd.fill(new FluidStack(recipe.recipeOutput.output.getFluid(), outputIntSnd), true);
				}
			}
			
			if(structured)
			{
				if(Math.abs((float)inputTank.getFluidAmount()/inputTank.getCapacity()-prevScale) > 0.01 && Math.abs((float)inputTankSnd.getFluidAmount()/inputTankSnd.getCapacity()-prevScaleSnd) > 0.01)
				{
					Mekanism.packetHandler.sendToReceivers(new TileEntityMessage(Coord4D.get(this), getNetworkedData(new ArrayList())), new Range4D(Coord4D.get(this)));
					prevScale = (float)inputTank.getFluidAmount()/inputTank.getCapacity();
                    prevScaleSnd = (float)inputTankSnd.getFluidAmount()/inputTankSnd.getCapacity();
				}
			}
		}
	}
	
	public GirdlerSulfidProcessorRecipe getRecipe()
	{
		return RecipeHandler.getGirdlerSulfidProcessorRecipe(new FluidInput(inputTank.getFluid()));
	}
	...
	public boolean hasRecipe(Fluid fluid)
	{
		...
		return Recipe.GIRDLER_SULFID_PROCESSOR.containsRecipe(fluid);
	}
	...
	public boolean canOperate(GirdlerSulfidProcessorRecipe recipe)
	{
		if(!structured || height < 3 || height > 18 || inputTank.getFluid() == null && inputTankSnd.getFluid() == null)
		{
			return false;
		}
		
		if(recipe != null && recipe.canOperate(inputTank, inputTankSnd, outputTank, outputTankSnd))
		{
			return true;
		}
	...
	/*
	 * Actually not supported by this Version. Eventually later.
	...
	*/
	...
	public float getMaxTemperature()
	{
		...
		return 1 + (height-3)*(float)general.girdlerSulfidProcessorSpeed;
	}
	...
	public boolean buildStructure()
	{
		...
		while(startPoint.getFromSide(ForgeDirection.UP).getTileEntity(worldObj) instanceof TileEntityGirdlerSulfidProcessorBlock)
		{
			startPoint.step(ForgeDirection.UP);
		}
		
		Coord4D test = startPoint.getFromSide(ForgeDirection.DOWN).getFromSide(right, 2);
		isLeftOnFace = test.getTileEntity(worldObj) instanceof TileEntityGirdlerSulfidProcessorBlock;
		...
		for(int x = 0; x < 4; x++)
		{
			for(int z = 0; z < 4; z++)
			{
				...
				if(corner != -1)
				{
					...
					else if(pointer.getFromSide(ForgeDirection.UP).getTileEntity(worldObj) instanceof TileEntityGirdlerSulfidProcessorBlock || !addTankPart(pointerTile))
					{
						return false;
					}
				}
					...
					if((x == 1 || x == 2) && (z == 1 || z == 2))
					{
						...
						if(pointer.getFromSide(ForgeDirection.UP).getTileEntity(worldObj) instanceof TileEntityGirdlerSulfidProcessorBlock || !addTankPart(pointerTile))
						{
							return false;
						}
					}
	...
	public boolean scanLowerLayer(Coord4D current)
	{
		...
		for(int x = 0; x < 4; x++)
		{
			for(int z = 0; z < 4; z++)
			{
				...
				if((x == 1 || x == 2) && (z == 1 || z == 2))
				{
					if(pointerTile instanceof TileEntityGirdlerSulfidProcessorBlock)
					{
						if(!foundCenter)
						{
							if(x == 1 && z == 1)
	...
	{
		if(tile instanceof TileEntityGirdlerSulfidProcessorBlock && (tile == this || !(tile instanceof TileEntityGirdlerSulfidProcessorController)))
		{
			if(tile != this)
			{
				((TileEntityGirdlerSulfidProcessorBlock)tile).addToStructure(Coord4D.get(this));
			...
			if(tile != this && tile instanceof TileEntityGirdlerSulfidProcessorController)
			{
    ...
    public int getScaledInputLevelSnd(int i)
    {
        return getMaxFluid() > 0 ? (inputTankSnd.getFluid() != null ? inputTankSnd.getFluid().amount*i / getMaxFluid() : 0) : 0;
    }

    public int getScaledOutputLevelSnd(int i)
    {
        return outputTankSnd.getFluid() != null ? outputTankSnd.getFluid().amount*i / MAX_OUTPUT : 0;
    }
		...
		if(structured != prev)
		{
			...
			if(structured)
			{
				Mekanism.proxy.doGenericSparkle(this, new INodeChecker() {
					...
					{
						return tile instanceof TileEntityGirdlerSulfidProcessorBlock;
					}
				});
			}
		}
	...
	public void clearStructure()
	{
		for(Coord4D tankPart : tankParts)
		{
			...
			if(tile instanceof TileEntityGirdlerSulfidProcessorBlock)
			{
				((TileEntityGirdlerSulfidProcessorBlock)tile).controllerGone();
			}
		}
	...
	{
		return new Object[] {inputTank, inputTankSnd, outputTank, outputTankSnd};
	}
}
