...
public class SyncableRegistryEntryList<V> extends SyncableList<V> {
    ...
    public static <V> SyncableRegistryEntryList<V> create(IForgeRegistry<V> registry, Supplier<@NonNull List<V>> getter, Consumer<@NonNull List<V>> setter) {
        return new SyncableRegistryEntryList<>(registry, getter, setter);
    }

    private final IForgeRegistry<V> registry;

    private SyncableRegistryEntryList(IForgeRegistry<V> registry, Supplier<@NonNull List<V>> getter, Consumer<@NonNull List<V>> setter) {
        super(getter, setter);
