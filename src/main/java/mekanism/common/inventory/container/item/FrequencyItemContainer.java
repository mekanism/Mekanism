...
public abstract class FrequencyItemContainer<FREQ extends Frequency> extends MekanismItemContainer {
    ...
    protected FrequencyItemContainer(ContainerTypeRegistryObject<?> type, int id, @Nullable PlayerInventory inv, Hand hand, ItemStack stack, FrequencyType<FREQ> frequencyType) {
    ...
    protected void openInventory(@Nonnull PlayerInventory inv) {
        ...
        if (!inv.player.world.isRemote()) {
            ...
            FrequencyManager<FREQ> manager = identity.isPublic() ? frequencyType.getManager(null) : frequencyType.getManager(inv.player.getUniqueID());
            FREQ freq = manager.getFrequency(identity.getKey());
            ...
            Mekanism.packetHandler.sendTo(PacketFrequencyItemGuiUpdate.create(hand, frequencyType, inv.player.getUniqueID(), freq), (ServerPlayerEntity) inv.player);
        }
    }
