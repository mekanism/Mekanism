...
public abstract class FrequencyItemContainer<FREQ extends Frequency> extends MekanismItemContainer {
    ...
    protected void addContainerTrackers() {
        ...
        if (isRemote()) {
            ...
            track(SyncableFrequencyList.create(() -> getFrequencyType().getManager(null, SecurityMode.PUBLIC).getFrequencies(), value -> publicCache = value));
            track(SyncableFrequencyList.create(() -> getFrequencyType().getManager(getPlayerUUID(), SecurityMode.PRIVATE).getFrequencies(), value -> privateCache = value));
            track(SyncableFrequencyList.create(() -> getFrequencyType().getManager(getPlayerUUID(), SecurityMode.TRUSTED).getFrequencies(), value -> trustedCache = value));
        }
    }
}