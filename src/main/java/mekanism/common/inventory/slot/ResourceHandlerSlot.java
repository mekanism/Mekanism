...
public abstract class ResourceHandlerSlot extends BasicInventorySlot {
    ...
    public void serialize(ValueOutput output) {
        ...
        //TODO - 26.3: Re-implement it and allow keeping track of it similar to how bins keep track of their lock type?
    ...
    private <RESOURCE extends Resource> ResourceHandler<RESOURCE> getHandler(InOutSlotResourceItemAccess<RESOURCE> access) {
        ...
        //TODO - 26.3: Can we somehow predict whether we should use a one by one access or not? The issue with not using a oneByOne access for things like buckets
        ...
        /*if (access.getAmount() > 1) {
            ...
            //TODO - 26.3: Determine if we should try extracting/inserting based on what we have available to see if we should potentially try the one by one access regardless?
        ...
        if (!fillContainerFromSlot(resourceContainer, handler, transaction)) {
            ...
            if (invalid) {
                ...
                try (Transaction subTransaction = Transaction.open(transaction)) {
                    ...
                    //TODO - 26.3: Should we be resetting it to unknown if we failed to move it?
                    return false;
                }
            }
        }
