...
public class ChemicalOxidizer
{
    ...
    public static void addRecipe(IItemStack itemInput, IGasStack gasOutput)
    {
        ...
        CraftTweakerAPI.apply(new AddMekanismRecipe(NAME, RecipeHandler.Recipe.CHEMICAL_OXIDIZER.get(), recipe));
    }
    ...
    public static void removeRecipe(IIngredient gasOutput, @Optional IIngredient itemInput)
    {
        ...
        if (!recipes.isEmpty())
        {
            CraftTweakerAPI.apply(new RemoveMekanismRecipe(NAME, RecipeHandler.Recipe.CHEMICAL_OXIDIZER.get(), recipes));
        }
        else
        {
