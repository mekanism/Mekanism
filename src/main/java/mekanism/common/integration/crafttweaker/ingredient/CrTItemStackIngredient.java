...
public class CrTItemStackIngredient {
    ...
    public static ItemStackIngredient from(IIngredientList ingredientList) {
        ...
        return IngredientCreatorAccess.item().from(CompoundIngredient.of(Arrays.stream(ingredients).map(IIngredient::asVanillaIngredient).toArray(Ingredient[]::new)));
    }
    ...
    public static IData asIData(ItemStackIngredient _this) {
        return ItemStackIngredient.CODEC.encodeStart(IDataOps.INSTANCE, _this).getOrThrow();
    }
    ...
    public static ItemStackIngredient or(ItemStackIngredient _this, ItemStackIngredient other) {
        if (_this.ingredient().count() != other.ingredient().count()) {
            throw new IllegalStateException("ItemStack ingredients can only be or'd if they have the same counts");
        }
        List<Ingredient> ingredients = new ArrayList<>();
        addIngredient(ingredients, _this.ingredient().ingredient());
        addIngredient(ingredients, other.ingredient().ingredient());
        return IngredientCreatorAccess.item().from(CompoundIngredient.of(ingredients.toArray(Ingredient[]::new)), _this.ingredient().count());
    }
