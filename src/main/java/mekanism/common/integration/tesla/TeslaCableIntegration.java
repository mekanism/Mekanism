...
public class TeslaCableIntegration implements ITeslaConsumer
{
	...
	{
		return (long)Math.round(tileEntity.acceptEnergy(side, power* MekanismConfig.current().general.FROM_TESLA.val(), simulated)* MekanismConfig.current().general.TO_TESLA.val());
	}
}
