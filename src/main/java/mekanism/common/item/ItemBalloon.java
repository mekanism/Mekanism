...
public class ItemBalloon extends ItemMekanism implements IMetaItem {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer entityplayer, @Nonnull EnumHand hand) {
        ...
        if (!world.isRemote) {
            Pos3D pos = new Pos3D(hand == EnumHand.MAIN_HAND ? -0.4 : 0.4, 0, 0.3)
                  .rotateYaw(entityplayer.renderYawOffset).translate(new Pos3D(entityplayer));
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase entity,
        ...
        if (player.isSneaking()) {
            if (!player.world.isRemote) {
                AxisAlignedBB bound = new AxisAlignedBB(entity.posX - 0.2, entity.posY - 0.5, entity.posZ - 0.2,
                      entity.posX + 0.2, entity.posY + entity.height + 4, entity.posZ + 0.2);
        ...
        public ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
            ...
            if (!latched) {
                ...
                if (!source.getWorld().isRemote) {
                    source.getWorld()
                          .spawnEntity(new EntityBalloon(source.getWorld(), pos.x, pos.y, pos.z, getColor(stack)));
                }
            }
