...
public abstract class ItemBlockMultipartAble extends ItemBlock {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, @Nonnull BlockPos pos, @Nonnull EnumHand hand,
        ...
        if (Mekanism.hooks.MCMPLoaded) {
            if (!block.isReplaceable(worldIn, pos) && !hasFreeMultiPartSpot(itemstack, worldIn, pos, iblockstate,
                  facing)) {//free spot handles case of no container
        ...
        if (player.canPlayerEdit(pos, facing, itemstack) && mayPlace(itemstack, worldIn, pos, iblockstate, hand,
              facing)) {
            ...
            boolean flag;
            if (Mekanism.hooks.MCMPLoaded) {
                flag = MultipartMekanism
                      .placeMultipartBlock(this.block, itemstack, player, worldIn, pos, facing, hitX, hitY, hitZ,
                            iblockstate1);
            } else {
    ...
    private boolean mayPlace(ItemStack itemstack, World worldIn, BlockPos pos, IBlockState state, EnumHand hand,
          EnumFacing facing) {
        if (!Mekanism.hooks.MCMPLoaded) {
            return worldIn.mayPlace(this.block, pos, false, facing, null);
        }
        return worldIn.mayPlace(this.block, pos, false, facing, null) || hasFreeMultiPartSpot(itemstack, worldIn, pos,
              state, facing);
    }

    private boolean hasFreeMultiPartSpot(ItemStack itemstack, World worldIn, BlockPos pos, IBlockState state,
          EnumFacing facing) {
        ...
        return container.get().canAddPart(slot, this.block
              .getStateForPlacement(worldIn, pos, facing, 0, 0, 0, itemstack.getMetadata(), null, EnumHand.MAIN_HAND));
    }
