...
///
/// This was introduced to fix [https://github.com/dizzyd/Mekanism/issues/2](https://github.com/dizzyd/Mekanism/issues/2). In that issue, another mod was trying to apply
/// a potion to the fake player and causing the whole system to crash due to essential potion related structures not being initialized for a fake player.
///
...
public class MekFakePlayer extends FakePlayer {
    ...
    /// Acquire a Fake Player and call a function which makes use of the player. Afterward, the Fake Player's world is nulled out to prevent GC issues. Emulated UUID is
    /// also reset.
    ///
    /// Do NOT store a reference to the Fake Player, so that it may be Garbage Collected. A fake player *should* only need to be short-lived
    ///
    ...
    /// Same as [MekFakePlayer#withFakePlayer(ServerLevel, java.util.function.Function)] but sets the Fake Player's position. Use when you think the entity position is
    /// relevant.
    ///
    /// @param world              World to set on the fake player
    /// @param fakePlayerConsumer consumer of the fake player
