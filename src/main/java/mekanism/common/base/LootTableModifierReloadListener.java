...
public class LootTableModifierReloadListener implements IResourceManagerReloadListener {//TODO - 1.17: Remove this won't be needed anymore
    ...
    public void onResourceManagerReload(@Nonnull IResourceManager resourceManager) {
        ...
        for (ResourceLocation id : lootTableManager.getIds()) {
            ...
            for (LootPool pool : FieldDetails.LOOT_TABLE_POOLS.getValue(lootTable)) {
                ...
                for (LootEntry entry : FieldDetails.LOOT_POOL_ENTRIES.getValue(pool)) {
                    modified |= modifyEntry(entry, shearsTag);
                }
                for (ILootCondition condition : FieldDetails.LOOT_POOL_CONDITIONS.getValue(pool)) {
        ...
        for (Map.Entry<ResourceLocation, ILootCondition> entry : FieldDetails.LOOT_PREDICATE_CONDITIONS.getValue(predicateManager).entrySet()) {
    ...
    private static boolean modifyEntry(LootEntry entry, ITag<Item> shearsTag) {
        ...
        if (entry instanceof ParentedLootEntry) {
            ...
            for (LootEntry child : FieldDetails.PARENTED_LOOT_ENTRY_CHILDREN.getValue((ParentedLootEntry) entry)) {
        ...
        modified |= modifyConditions(FieldDetails.LOOT_ENTRY_CONDITIONS.getValue(entry), shearsTag);
    ...
    private static boolean modifyCondition(ILootCondition condition, ITag<Item> shearsTag) {
        if (condition instanceof Alternative) {
            ...
            return modifyConditions(FieldDetails.ALTERNATIVE_CONDITION.getValue((Alternative) condition), shearsTag);
            ...
            return modifyConditions(FieldDetails.LOOT_PREDICATE_AND_CONDITION.getValue((LootPredicateManager.AndCombiner) condition), shearsTag);
            ...
            ILootCondition inner = FieldDetails.INVERTED_CONDITION.getValue((Inverted) condition);
            ...
            if (FieldDetails.ITEM_PREDICATE_ITEM.getValue(predicate) == Items.SHEARS || FieldDetails.ITEM_PREDICATE_TAG.getValue(predicate) == shearsTag) {
    ...
    private static class ShearsItemPredicate extends ItemPredicate {
        ...
        private ShearsItemPredicate(ItemPredicate inner) {
            ...
            super(null, null, FieldDetails.ITEM_PREDICATE_COUNT.getValue(inner), FieldDetails.ITEM_PREDICATE_DURABILITY.getValue(inner),
                  FieldDetails.ITEM_PREDICATE_ENCHANTMENTS.getValue(inner), FieldDetails.ITEM_PREDICATE_STORED_ENCHANTMENTS.getValue(inner),
                  FieldDetails.ITEM_PREDICATE_POTION.getValue(inner), FieldDetails.ITEM_PREDICATE_NBT.getValue(inner));
    ...
    private static class FieldDetails<CLASS, TYPE> {
        ...
        //The following could in theory be in ATs but aren't because we need to have reflection here anyways so we may as well do it all via reflection
        private static final FieldDetails<LootEntry, ILootCondition[]> LOOT_ENTRY_CONDITIONS = new FieldDetails<>(LootEntry.class, "field_216144_d", () -> new ILootCondition[0]);
        private static final FieldDetails<ParentedLootEntry, LootEntry[]> PARENTED_LOOT_ENTRY_CHILDREN = new FieldDetails<>(ParentedLootEntry.class, "field_216147_c", () -> new LootEntry[0]);
        private static final FieldDetails<Alternative, ILootCondition[]> ALTERNATIVE_CONDITION = new FieldDetails<>(Alternative.class, "field_215962_a", () -> new ILootCondition[0]);
        private static final FieldDetails<Inverted, ILootCondition> INVERTED_CONDITION = new FieldDetails<>(Inverted.class, "field_215981_a", () -> null);
        private static final FieldDetails<ItemPredicate, ITag<Item>> ITEM_PREDICATE_TAG = new FieldDetails<>(ItemPredicate.class, "field_200018_b", () -> null);
        private static final FieldDetails<ItemPredicate, Item> ITEM_PREDICATE_ITEM = new FieldDetails<>(ItemPredicate.class, "field_192496_b", () -> null);
        private static final FieldDetails<ItemPredicate, MinMaxBounds.IntBound> ITEM_PREDICATE_COUNT = new FieldDetails<>(ItemPredicate.class, "field_192498_d", () -> MinMaxBounds.IntBound.ANY);
        private static final FieldDetails<ItemPredicate, MinMaxBounds.IntBound> ITEM_PREDICATE_DURABILITY = new FieldDetails<>(ItemPredicate.class, "field_193444_e", () -> MinMaxBounds.IntBound.ANY);
        private static final FieldDetails<ItemPredicate, EnchantmentPredicate[]> ITEM_PREDICATE_ENCHANTMENTS = new FieldDetails<>(ItemPredicate.class, "field_192499_e", () -> EnchantmentPredicate.NONE);
        private static final FieldDetails<ItemPredicate, EnchantmentPredicate[]> ITEM_PREDICATE_STORED_ENCHANTMENTS = new FieldDetails<>(ItemPredicate.class, "field_226656_g_", () -> EnchantmentPredicate.NONE);
        private static final FieldDetails<ItemPredicate, Potion> ITEM_PREDICATE_POTION = new FieldDetails<>(ItemPredicate.class, "field_192500_f", () -> null);
        private static final FieldDetails<ItemPredicate, NBTPredicate> ITEM_PREDICATE_NBT = new FieldDetails<>(ItemPredicate.class, "field_193445_h", () -> NBTPredicate.ANY);
        ...
        private static final FieldDetails<LootPredicateManager.AndCombiner, ILootCondition[]> LOOT_PREDICATE_AND_CONDITION = new FieldDetails<>(LootPredicateManager.AndCombiner.class, "field_237405_a_", () -> new ILootCondition[0]);
        ...
        private FieldDetails(Class<CLASS> clazz, String fieldName, Supplier<TYPE> fallback) {
            this.clazz = clazz;
