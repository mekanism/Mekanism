...
public class ModuleDeferredRegister extends MekanismDeferredRegister<ModuleData<?>> {
    ...
    public ModuleRegistryObject<?> registerMarker(String name) {
        return registerMarker(name, UnaryOperator.identity());
    }

    public ModuleRegistryObject<?> registerMarker(String name, UnaryOperator<ModuleDataBuilder<?>> builderModifier) {
        return registerBuilder(name, () -> builderModifier.apply(ModuleDataBuilder.marker()));
    }
    ...
    public <MODULE extends ICustomModule<MODULE>> ModuleRegistryObject<MODULE> register(String name, Function<IModule<MODULE>, MODULE> constructor,
        ...
        return registerBuilder(name, () -> builderModifier.apply(ModuleDataBuilder.custom(constructor)));
    }

    //Do not use this method if you have any config options
    public ModuleRegistryObject<SimpleEnchantmentAwareModule> registerEnchantBased(String name, ResourceKey<Enchantment> enchantment,
          UnaryOperator<ModuleDataBuilder<SimpleEnchantmentAwareModule>> builderModifier) {
        return registerInstanced(name, () -> new SimpleEnchantmentAwareModule(enchantment), builderModifier);
    }
    ...
    public <MODULE extends ICustomModule<MODULE>> ModuleRegistryObject<MODULE> registerInstanced(String name, Supplier<MODULE> constructor,
          UnaryOperator<ModuleDataBuilder<MODULE>> builderModifier) {
        return registerBuilder(name, () -> builderModifier.apply(ModuleDataBuilder.customInstanced(constructor)));
    }
