...
public class BlockEnrichmentChamber extends BlockMekanismContainer implements IBlockMekanism {

    private final Predicate<EnumFacing> facingPredicate;
    private final String name;

    public BlockEnrichmentChamber(String name, Predicate<EnumFacing> facingPredicate) {
        super(Material.IRON);
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer entityplayer, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (tileEntity != null) {
            if (!entityplayer.isSneaking()) {
                if (SecurityUtils.canAccess(entityplayer, tileEntity)) {
                    entityplayer.openGui(Mekanism.instance, MachineType.ENRICHMENT_CHAMBER.guiId, world, pos.getX(), pos.getY(), pos.getZ());
                } else {
                    SecurityUtils.displayNoAccess(entityplayer);
                }
                return true;
