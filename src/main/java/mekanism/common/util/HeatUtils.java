    ...
    public static double[] simulate(IHeatTransfer source) {
        ...
        for (EnumFacing side : EnumFacing.VALUES) {
            ...
            if (sink != null) {
                ...
                if (!(sink instanceof ICapabilityProvider && CapabilityUtils
                      .hasCapability((ICapabilityProvider) sink, Capabilities.GRID_TRANSMITTER_CAPABILITY,
                            side.getOpposite()))) {
            ...
            double invConduction =
                  IHeatTransfer.AIR_INVERSE_COEFFICIENT + source.getInsulationCoefficient(side) + source
                        .getInverseConductionCoefficient();
            double heatToTransfer = source.getTemp() / invConduction;
            source.transferHeatTo(-heatToTransfer);
