...
//		}
	...
	/**
	 * Clamp a double to int without using Math.min due to double representation issues.
	 * Primary use: power systems that use int, where Mek uses doubles internally
	 *
	 * <code>
	 * double d = 1e300; // way bigger than longs, so the long should always be what's returned by Math.min
	 * System.out.println((long)Math.min(123456781234567812L, d)); // result is 123456781234567808 - 4 less than what you'd expect
	 * System.out.println((long)Math.min(123456789012345678L, d)); // result is 123456789012345680 - 2 more than what you'd expect
	 * </code>
	 *
	...
	/**
	 * Clamp a double to long without using Math.min due to double representation issues.
	 * Primary use: power systems that use long, where Mek uses doubles internally
	 *
	 * @param d double to clamp
