            ...
            if (stack.getItem() instanceof IEnergizedItem) {
                ...
                if (storage.canExtract()) {
                    int needed = (int) Math.round(Math.min(Integer.MAX_VALUE,
                          (storer.getMaxEnergy() - storer.getEnergy()) * MekanismConfig.current().general.TO_FORGE
                                .val()));
                ...
                int needed = (int) Math.round(Math.min(Integer.MAX_VALUE,
                      (storer.getMaxEnergy() - storer.getEnergy()) * MekanismConfig.current().general.TO_RF.val()));
                ...
                double gain = ElectricItem.manager.discharge(stack,
                      (storer.getMaxEnergy() - storer.getEnergy()) * MekanismConfig.current().general.TO_IC2.val(), 4,
                      true, true, false) * MekanismConfig.current().general.FROM_IC2.val();
        ...
        if (!stack.isEmpty() && storer.getEnergy() > 0) {
            if (stack.getItem() instanceof IEnergizedItem) {
                ...
                if (storage.canReceive()) {
                    int stored = (int) Math.round(Math.min(Integer.MAX_VALUE,
                          storer.getEnergy() * MekanismConfig.current().general.TO_FORGE.val()));
