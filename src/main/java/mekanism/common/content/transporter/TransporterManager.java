...
public class TransporterManager {
        ...
        for (TransporterStack stack : flowing.stacks) {
            if (!stack.isEmpty() && stack.getPathType().hasTarget()) {
                ...
                if (inserted < toInsert) {
                    //TODO - 26.3: Should we be failing here? Because what if we can't insert the apples that are en route, but can insert the carrots
                    if (inserted == 0) {
    ...
    private static class FlowingJournal extends SnapshotJournal<Set<TransporterStack>> {
        ...
        //TODO - 26.3: Should this be some sort of queue, or should we do reverting similar to how it works for dropped items?
        ...
        private void add(TransporterStack stack, @Nullable TransactionContext transaction) {
            //TODO - 26.3: If we are in a transaction, but aren't passed one, then this might get rolled back by revert?
