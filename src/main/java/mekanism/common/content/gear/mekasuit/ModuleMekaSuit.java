...
public abstract class ModuleMekaSuit extends Module {
	...
	public static class ModuleElectrolyticBreathingUnit extends ModuleMekaSuit {
		...
		public void tickServer(PlayerEntity player) {
			...
			// Note: Being in water is checked first to ensure that if it is raining and the
			// player is in water
			...
			if (fluidstate.isTagged(FluidTags.WATER)
					&& maskHeight <= headPos.getY() + fluidstate.getActualHeight(player.getEntityWorld(), headPos)) {
				// If the position the bottom of the mask is in is water set the production rate
				// to our max rate
			...
			if (productionRate > 0) {
				...
				Optional<IGasHandler> chestCapability = chestStack.getCapability(Capabilities.GAS_HANDLER_CAPABILITY)
						.resolve();
				if (checkChestPlate(chestStack) && chestCapability.isPresent()) {
					hydrogenUsed = maxRate * 2L
							- chestCapability.get().insertChemical(hydrogenStack, Action.EXECUTE).getAmount();
					hydrogenStack.shrink(hydrogenUsed);
				}
				if (fillHeld.get()) {
					ItemStack handStack = player.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
					Optional<IGasHandler> handCapability = handStack.getCapability(Capabilities.GAS_HANDLER_CAPABILITY)
							.resolve();
					if (handCapability.isPresent()) {
						hydrogenUsed = maxRate * 2L
								- handCapability.get().insertChemical(hydrogenStack, Action.EXECUTE).getAmount();
					}
				}
		...
		/**
		 * Checks whether the given chestplate should be filled with hydrogen, if it can
		 * store hydrogen. Does not check whether the chestplate can store hydrogen.
		 *
	...
	public static class ModuleInhalationPurificationUnit extends ModuleMekaSuit {
		...
		public void init() {
			super.init();
			addConfigItem(beneficialEffects = new ModuleConfigItem<>(this, "beneficial_effects",
					MekanismLang.MODULE_PURIFICATION_BENEFICIAL, new BooleanData(), false));
			addConfigItem(neutralEffects = new ModuleConfigItem<>(this, "neutral_effects",
					MekanismLang.MODULE_PURIFICATION_NEUTRAL, new BooleanData(), true));
			addConfigItem(harmfulEffects = new ModuleConfigItem<>(this, "harmful_effects",
					MekanismLang.MODULE_PURIFICATION_HARMFUL, new BooleanData(), true));
		}

		@Override
		public void tickClient(PlayerEntity player) {
			super.tickClient(player);
			// Messy rough estimate version of tickServer so that the timer actually
			// properly updates
			if (!player.isSpectator()) {
				...
				if (free || energy.greaterOrEqual(usage)) {
					// Gather all the active effects that we can handle, so that we have them in
					// their own list and
					...
					for (EffectInstance effect : effects) {
						if (free) {
							...
							if (energy.smallerThan(usage)) {
								// If after using energy, our remaining energy is now smaller than how much we
								// need to use, exit
								break;
							}
						}
		...
		public void tickServer(PlayerEntity player) {
			...
			if (free || (energyContainer != null && energyContainer.getEnergy().greaterOrEqual(usage))) {
				...
				// don't run into any issues related to CMEs
				List<EffectInstance> effects = player.getActivePotionEffects().stream()
						.filter(effect -> canHandle(effect.getPotion().getEffectType())).collect(Collectors.toList());
				for (EffectInstance effect : effects) {
					if (free) {
						speedupEffect(player, effect);
	...
	public static class ModuleVisionEnhancementUnit extends ModuleMekaSuit {
		...
		private static final ResourceLocation icon = MekanismUtils.getResource(ResourceType.GUI_HUD,
				"vision_enhancement_unit.png");
		...
		@Override
		public void changeMode(@Nonnull PlayerEntity player, @Nonnull ItemStack stack, int shift,
				boolean displayChangeMessage) {
	...
	public static class ModuleGravitationalModulatingUnit extends ModuleMekaSuit {
		...
		private static final ResourceLocation icon = MekanismUtils.getResource(ResourceType.GUI_HUD,
				"gravitational_modulation_unit.png");
		...
		public void init() {
			super.init();
			addConfigItem(speedBoost = new ModuleConfigItem<>(this, "speed_boost", MekanismLang.MODULE_SPEED_BOOST,
					new EnumData<>(SprintBoost.class), SprintBoost.LOW));
		}
	...
	public static class ModuleHydraulicPropulsionUnit extends ModuleMekaSuit {
		...
		public void init() {
			super.init();
			addConfigItem(jumpBoost = new ModuleConfigItem<>(this, "jump_boost", MekanismLang.MODULE_JUMP_BOOST,
					new EnumData<>(JumpBoost.class, getInstalledCount() + 1), JumpBoost.LOW));
			addConfigItem(stepAssist = new ModuleConfigItem<>(this, "step_assist", MekanismLang.MODULE_STEP_ASSIST,
					new EnumData<>(StepAssist.class, getInstalledCount() + 1), StepAssist.LOW));
		}
		...
		public enum JumpBoost implements IHasTextComponent {
			OFF(0), LOW(0.5F), MED(1), HIGH(3), ULTRA(5);
		...
		public enum StepAssist implements IHasTextComponent {
			OFF(0), LOW(0.5F), MED(1), HIGH(1.5F), ULTRA(2);
	...
	public static class ModuleSolarRechargingUnit extends ModuleMekaSuit {
		...
		public void tickServer(PlayerEntity player) {
			...
			if (energyContainer != null && !energyContainer.getNeeded().isZero() && player.world.isDaytime()
					&& player.world.canSeeSky(new BlockPos(player.getPositionVec())) && !player.world.isRaining()
					&& player.world.getDimensionType().hasSkyLight()) {
	...
	public static class ModuleNutritionalInjectionUnit extends ModuleMekaSuit {
		...
		private static final ResourceLocation icon = MekanismUtils.getResource(ResourceType.GUI_HUD,
				"nutritional_injection_unit.png");
		...
		public void tickServer(PlayerEntity player) {
			...
			if (MekanismUtils.isPlayingMode(player) && player.canEat(false)
					&& getContainerEnergy().greaterOrEqual(usage)) {
				ItemMekaSuitArmor item = (ItemMekaSuitArmor) getContainer().getItem();
				long toFeed = Math.min(1,
						item.getContainedGas(getContainer(), MekanismGases.NUTRITIONAL_PASTE.get()).getAmount()
								/ MekanismConfig.general.nutritionalPasteMBPerFood.get());
				if (toFeed > 0) {
					useEnergy(player, usage.multiply(toFeed));
					item.useGas(getContainer(), MekanismGases.NUTRITIONAL_PASTE.get(),
		...
		public void addHUDElements(List<HUDElement> list) {
			...
			GasStack stored = ((ItemMekaSuitArmor) getContainer().getItem()).getContainedGas(getContainer(),
					MekanismGases.NUTRITIONAL_PASTE.get());
			double ratio = StorageUtils.getRatio(stored.getAmount(),
					MekanismConfig.gear.mekaSuitNutritionalMaxStorage.getAsLong());
	...
	public static class ModuleDosimeterUnit extends ModuleMekaSuit {
		...
		public void addHUDElements(List<HUDElement> list) {
			...
			Optional<IRadiationEntity> capability = CapabilityUtils
					.getCapability(Minecraft.getInstance().player, Capabilities.RADIATION_ENTITY_CAPABILITY, null)
					.resolve();
			if (capability.isPresent()) {
				...
				HUDColor color = radiation < RadiationManager.MIN_MAGNITUDE ? HUDColor.REGULAR
						: (radiation < 0.1 ? HUDColor.WARNING : HUDColor.DANGER);
