        ...
        for (int tableIndex = 0; tableIndex < craftingWindows.length; tableIndex++) {
            ...
            QIOCraftingWindow craftingWindow = new QIOCraftingWindow(this, (byte) tableIndex, slot -> () -> {
                ...
                if (!initializing) {
                    ...
                    stack.set(MekanismDataComponents.QIO_DASHBOARD, content.with(finalTableIndex, slot, inputSlot.resource(), inputSlot.amountAsLong()));
                }
            });
            ...
            for (int slot = 0; slot < 9; slot++) {
                ...
                slots.add(inputSlot);
                inputSlot.setContentsUnchecked(contents.get(tableIndex * 9 + slot));
            }
