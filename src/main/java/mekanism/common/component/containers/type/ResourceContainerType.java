...
public abstract class ResourceContainerType<RESOURCE extends Resource, CONTAINER extends IResourceContainer<RESOURCE>>
    ...
    protected ResourceHandler<RESOURCE> createHandler(ItemAccess itemAccess) {
        ...
        if (attached == null || attached.isEmpty()) {
            ...
            //TODO - 26.3: Re-evaluate this branch not just being zero and then returning null is the only difference for what getCapOrUnexposed
    ...
    public void addStoredResource(AttachedResources<RESOURCE> containers, Consumer<Component> builder, ILangEntry emptyLangEntry, EnumColor color,
          @Nullable IStorageTier tier, @Nullable ILangEntry displayType, boolean forceAsList, ILangEntry listHeader) {
        if (containers.hasNonEmptyContents()) {
            if (forceAsList || containers.size() > 1) {
                builder.accept(listHeader.translateColored(EnumColor.YELLOW));
            }
            boolean isCreative = tier != null && tier.isCreative();
            if (containers.size() == 1) {
                //Note: We know this resource is not empty here
                Component tooltip = getStoredResourceTooltip(containers.get(0), color, isCreative, displayType);
                if (forceAsList) {
                    tooltip = MekanismLang.GENERIC_LIST.translate(tooltip);
                }
                builder.accept(tooltip);
            } else {
                for (LargeResourceStack<RESOURCE> resource : containers) {
                    if (!resource.isEmpty()) {
                        builder.accept(MekanismLang.GENERIC_LIST.translate(getStoredResourceTooltip(resource, color, isCreative, displayType)));
                    }
                }
            }
        } else if (!containers.isEmpty()) {
            builder.accept(emptyLangEntry.translateColored(EnumColor.DARK_RED));
        }
    }

    private Component getStoredResourceTooltip(LargeResourceStack<RESOURCE> resource, EnumColor color, boolean isCreative, @Nullable ILangEntry displayType) {
        Component tooltip;
        if (isCreative) {
    ...
    public void merge(List<CONTAINER> orig, List<CONTAINER> toAdd, Object2LongMap<RESOURCE> rejects, TransactionContext transaction) {
        ...
        for (int container = 0, size = toAdd.size(); container < size; container++) {
            ...
            if (!toAddContainer.isEmpty()) {
                ...
                //TODO - 26.3: Validate all callers have this work with the given automation type
                //TODO - 26.3: Should we change this to bypass all rate limits in case we add a multiblock that has them at a later date.
                // That might be problematic to do, as the container merging uses dummy containers, that have no limits
                //TODO - 26.3: If toAddAmount is greater than max int how do we want to handle it
    ...
    public boolean interactWithHandler(Player player, InteractionHand hand, @Nullable BlockPos pos, ResourceHandler<RESOURCE> handler, @Nullable TransactionContext transaction) {
        //TODO - 26.3: Should we add a variant of this that allows following a container edit mode?
        ...
        //TODO - 26.3: Do we want chemical handler interactions to fire game events or make sounds?
