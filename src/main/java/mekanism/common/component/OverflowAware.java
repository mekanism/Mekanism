...
public record OverflowAware(Object2IntSortedMap<ItemResource> overflow) {
    ...
    public static final Codec<OverflowAware> CODEC = ItemCount.CODEC.listOf()
          .promotePartial(error -> Mekanism.logger.error("Failed to load overflown items: {}", error)).xmap(
                counts -> {
                    Object2IntSortedMap<ItemResource> overflow = new Object2IntLinkedOpenHashMap<>(counts.size());
                    ...
                    for (ObjectIterator<Entry<ItemResource>> iterator = Object2IntMaps.fastIterator(overflowAware.overflow()); iterator.hasNext(); ) {
                        Object2IntMap.Entry<ItemResource> entry = iterator.next();
                        counts.add(new ItemCount(entry.getKey(), entry.getIntValue()));
                    }
                    return counts;
                });
    public static final StreamCodec<RegistryFriendlyByteBuf, OverflowAware> STREAM_CODEC = ByteBufCodecs.<RegistryFriendlyByteBuf, ItemResource, Integer, Object2IntSortedMap<ItemResource>>map(
          Object2IntLinkedOpenHashMap::new, ItemResource.STREAM_CODEC, ByteBufCodecs.VAR_INT
    ...
    private record ItemCount(ItemResource type, int count) {

        public static final Codec<ItemCount> CODEC = RecordCodecBuilder.create(instance -> instance.group(
              ItemResource.CODEC.fieldOf(SerializationConstants.TYPE).forGetter(ItemCount::type),
