...
/// Helper class for creating builtin implementations of [RadialData].
///
/// @see IRadialDataHelper#INSTANCE
/// @since 10.3.2
public interface IRadialDataHelper {

    /// Provides access to Mekanism's implementation of [IRadialDataHelper].
    ///
    /// @since 10.4.0
    ...
    ///
    /// @apiNote Does not currently support [mekanism.api.IDisableableEnum].
    ...
    /// Creates a Boolean based Radial Data implementation with a default mode corresponding to [BooleanRadialModes#falseMode].
    ///
    /// @param identifier Identifier representing the radial data. Must be unique within the radial level if it will be used as a nested radial element.
    ...
    ///
    /// @param falseMode Radial mode corresponding to a `false` value.
    /// @param trueMode  Radial mode corresponding to a `true` value.
    record BooleanRadialModes(IRadialMode falseMode, IRadialMode trueMode) {
        ...
        ///
        /// @return [#falseMode] if value is `false`, otherwise [#trueMode]
