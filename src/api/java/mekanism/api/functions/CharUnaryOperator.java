...
/// Represents an operation on a single `char`-valued operand that produces a `char`-valued result.  This is the primitive type specialization of [UnaryOperator] for
/// `char`.
///
/// This is a [functional interface](package-summary.html) whose functional method is [#applyAsChar(char)].
///
/// @see UnaryOperator
@FunctionalInterface
public interface CharUnaryOperator {
    ...
    /// Returns a composed operator that first applies the `before` operator to its input, and then applies this operator to the result. If evaluation of either operator
    ...
    /// Returns a composed operator that first applies this operator to its input, and then applies the `after` operator to the result. If evaluation of either operator
    /// throws an exception, it is relayed to the caller of the composed operator.
    ///
