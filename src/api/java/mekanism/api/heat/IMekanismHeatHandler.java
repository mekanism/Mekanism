...
public interface IMekanismHeatHandler extends ISidedHeatHandler {
    ...
    /// Used to check if an instance of [IMekanismHeatHandler] actually has the ability to handle heat.
    ///
    /// @return True if we are actually capable of handling heat.
    ///
    /// @apiNote If for some reason you are comparing to [IMekanismHeatHandler] without having gotten the object via the heat handler capability, then you must call this
    /// method to make sure that it really can handle heat. As most mekanism tiles have this class in their hierarchy.
    ...
    ///
    /// @return The list of all IHeatCapacitors that this [IMekanismHeatHandler] contains for the given side. If there are no capacitors for the side or
    /// [#canHandleHeat()] is false then it returns an empty list.
    ///
    /// @implNote When side is null (an internal request), this method _MUST_ return all capacitors in the handler. Additionally, if [#canHandleHeat()] is false, this
    /// _MUST_ return an empty list.
