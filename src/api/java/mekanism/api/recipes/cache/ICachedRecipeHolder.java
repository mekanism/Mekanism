...
public interface ICachedRecipeHolder<RECIPE extends MekanismRecipe<?>> {
    ...
    /// @param cacheIndex The "recipe index" for which cache to interact with.
    ///
    /// @return Current cached recipe or `null` if no recipe is cached or the holder does not handle the given cacheIndex.
    @Nullable
    ...
    /// @param cacheIndex The "recipe index" for which cache to interact with.
    ///
    /// @return `true` if we know we have no recipe, as nothing has changed since we last checked for a recipe, `false` if we may have a recipe or the implementation does
    /// not make use of this extra caching.
