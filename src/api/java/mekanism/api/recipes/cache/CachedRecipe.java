...
public abstract class CachedRecipe<RECIPE extends MekanismRecipe<?>> {
    ...
    /// Set of all the errors from when this [CachedRecipe] last calculated all the errors.
    ...
    /// Used to check if the [CachedRecipe] should recheck for all errors.
    private final BooleanSupplier recheckAllErrors;

    /// Used to check if the [CachedRecipe] holder can function.
    ///
    /// @implNote Defaults to returning `true`.
    ...
    /// Gets the energy currently stored in the machine/object executing this [CachedRecipe].
    ///
    ...
    /// Function to allow for post-processing [#calculateOperationsThisTick(OperationTracker)].
    ///
    /// @implNote Defaults to doing nothing.
    ...
    /// Allows for cached recipe holders to have handling for when the operating ticks changed, for example to allow the machine to know that the number of operating
    /// ticks has changed and sync the progress in containers to clients.
    ///
    /// @implNote Defaults to doing nothing.
    ...
    /// this every tick or if there is no one viewing recipes.
    ...
    /// Sets the callback that is used to check if the [CachedRecipe] holder is currently able to function. If the holder is not able to function, it will be set to
    ...
    ///
    /// @param canHolderFunction Returns `true` If the holder can function, or `false` if it can't.
    ///
    /// @apiNote If this method is not used, `canHolderFunction` for this [CachedRecipe] defaults to returning true.
    ...
    /// Sets the callback that is used to change the active state of the [CachedRecipe] holder.
    ///
    /// @param setActive Consumer that handles changing the active state of the holder.
    ///
    /// @apiNote If this method is not used, the `setActive` consumer of this [CachedRecipe] defaults to doing nothing.
    ...
    /// Sets the various energy requirements of this [CachedRecipe] and allows for gathering enough information to allow for calculating and using the stored energy.
    ///
    ...
    ///
    /// @apiNote If this method is not used, this [CachedRecipe] defaults to not requiring or using any energy.
    public CachedRecipe<RECIPE> setEnergyRequirements(IntSupplier perTickEnergy, IEnergyContainer energyContainer) {
        ...
        this.storedEnergy = energyContainer::getAmountAsInt;
    ...
    ///
    /// @apiNote `requiredTicks` should return a value of at least one, though any lower values will be treated as if the recipe takes a single tick to complete.
    ///
    /// If this method is not used, the `requiredTicks` of this [CachedRecipe] defaults to returning one.
    ...
    /// Sets the callback used for handling and keeping track of when the number of operating ticks for this [CachedRecipe] changes. This can be used to allow the
    /// [CachedRecipe] holder to persist the recipe's progress between sessions and to ensure the proper value gets synced to the client in GUIs.
    ///
    /// @param operatingTicksChanged Called when the number of operating ticks changes with the new operating tick number.
    ///
    /// @apiNote If this method is not used, the `operatingTicksChanged` consumer of this [CachedRecipe] defaults to doing nothing.
    ...
    /// Sets the callback that is run when this [CachedRecipe] completes processing of a recipe. This allows the [CachedRecipe] holder to do any extra handling it needs
    /// to do such as marking the tile dirty and in need of saving.
    ///
    /// @param onFinish Runnable to execute when this [CachedRecipe] completes processing of a recipe.
    ///
    /// @apiNote If this method is not used, the `onFinish` handler of this [CachedRecipe] defaults to doing nothing.
    ...
    ///
    /// @apiNote `baselineMaxOperations` should return a value of at least one.
    ///
    /// If this method is not used, the `baselineMaxOperations` of this [CachedRecipe] defaults to returning one.
    ...
    /// Sets the callback used to post-process the number of operations this [CachedRecipe] calculated it can perform in [#calculateOperationsThisTick(OperationTracker)]
    /// and to add any [RecipeError]s that occur during post-processing.
    ///
    /// It can be assumed that when `postProcessOperations` is called [OperationTracker#shouldContinueChecking()] is `true`.
    ///
    /// @param postProcessOperations Function that applies post-processing to the result of [#calculateOperationsThisTick(OperationTracker)].
    ///
    /// @apiNote If this method is not used, `postProcessOperations` for this [CachedRecipe] defaults to doing nothing as the baseline already has the maximum amount of
    /// operations we can actually perform.
    ...
    /// Sets the callback that run when the set of known [RecipeError]s of this [CachedRecipe] changes.
    ///
    /// @param onErrorsChange Consumer to call with the set of [RecipeError]s this [CachedRecipe] when they change.
    ///
    /// @apiNote If this method is not used, this [CachedRecipe] defaults to not notifying any holder when the errors change.
    ...
    /// Updates the known errors to the given set, and calls [#onErrorsChange] if the errors actually changed.
    ...
    /// Sets the number of operating ticks that have passed so far. This is used to allow [CachedRecipe] holders to persist and load recipe progress.
    ///
    ...
    /// Called to allow recipes to set up any variables they will need this tick to allow them to reduce the number of places they have to recalculate/retrieve a passed
    /// in value.
    ...
    /// Called each tick to allow for [CachedRecipe]s to consume any per tick resources.
    ///
    /// @param operations  Number of operations being performed.
    /// @param transaction The transaction that this operation is part of.
    ///
    /// @return `true` if enough resources was used.
    ///
    /// @implNote It is safe to assume that [#calculateOperationsThisTick(OperationTracker)] will have been called before this method and there will be at least one
    /// operation being performed. This means that caching of types can be done inside of [#calculateOperationsThisTick(OperationTracker)] and safely used here.
    ...
    /// @param transaction The transaction that this operation is part of.
    ///
    /// @return `true` if enough energy was used.
    ...
    /// Calculates the number of operations this [CachedRecipe] can perform this tick based on the various inputs available to the recipe and space left in the outputs
    /// and updates the given operation tracker. If this [CachedRecipe] runs into any problems while processing it should also track the corresponding [RecipeError] using
    ...
    /// Checks if the resources in the inputs is valid for this [CachedRecipe].
    ///
    /// @return `true` if the resources are valid.
    ...
    /// @param transaction The transaction that this operation is part of.
    ///
    /// @return `true` if inputs were consumed and outputs added.
    ///
    /// @implNote It is safe to assume that [#calculateOperationsThisTick(OperationTracker)] will have been called before this method and there will be at least one
    /// operation being performed. This means that caching of types and outputs can be done inside of [#calculateOperationsThisTick(OperationTracker)] and safely used
    /// here.
    protected abstract boolean finishProcessing(int operations, TransactionContext transaction);

    /// Gets the actual recipe object this [CachedRecipe] has cached and operates on.
    ...
    /// Class for keeping track of the number of max operations that can be performed by the [CachedRecipe], and any errors that it has run into while calculating how
    /// many operations it can perform at once.
    ///
    ...
    public static final class OperationTracker {
        ...
        /// Set of all the errors from when the [CachedRecipe] last calculated the complete set of errors.
        private final Set<RecipeError> lastErrors;
        /// Set of errors that the [CachedRecipe] has run into so far while calculating how many operations it can perform at once.
        private Set<RecipeError> errors = Collections.emptySet();
        /// Used to determine if this tracker should try and check for all existing errors (`true`) or short circuit as soon as it knows no processing can be done.
        private boolean checkAll;
        /// Caches whether we have checked if [#lastErrors] contains all [#errors].
        ///
        /// @implNote Starts at `true` as [#lastErrors] will always contain the contents of an empty set (the default for new [#errors]).
        private boolean checkedErrors = true;
        /// The current maximum number of operations that the [CachedRecipe] can perform this tick.
        private int currentMax;
        /// The current maximum number of operations that the [CachedRecipe] can perform this tick due to available energy.
        private int maxForEnergy;

        /// @param lastErrors     Set of the last errors the [CachedRecipe] had.
        /// @param checkAll`true` if this tracker should try and check for all existing errors.
        /// @param startingMax    Starting maximum number of operations that the [CachedRecipe] can perform this tick.
        ...
        /// @return `true` if the errors from this [OperationTracker] should be copied to the [ICachedRecipeHolder].
        ...
        /// @return `true` if the [CachedRecipe] should continue calculating how many operations can be performed this tick.
        ///
        /// @see CachedRecipe#calculateOperationsThisTick(OperationTracker)
        ...
        /// Updates the maximum number of operations the [CachedRecipe] can currently perform.
        ///
        ...
        ///
        /// @return `true` if the maximum number of operations decreased as a result of this method call.
        ///
        /// @apiNote This method can only reduce the maximum number of operations, it cannot increase it.
        ///
        /// It is recommended to use one of the specific reset methods ([#mismatchedRecipe()], [#resetProgress(RecipeError)]) rather than using this method to reset the
        /// recipe's progress.
        ...
        /// Updates the maximum number of operations the [CachedRecipe] can currently perform to the maximum number that was calculated based on available energy.
        ///
        /// @return `true` if the maximum number of operations decreased as a result of this method call.
        ///
        /// @apiNote This method can only reduce the maximum number of operations, it cannot increase it.
        ...
        /// Marks that the recipe's progress should be reset and adds the given [RecipeError] to the set of errors that caused the recipe to reset.
        ///
        ...
        /// Adds a [RecipeError] to the set of errors stopping this recipe from operating at peak efficiency (or potentially at all).
        ///
        /// @param error Error to add.
        ...
        /// Marker class for errors [CachedRecipe]s may run into when processing a recipe.
