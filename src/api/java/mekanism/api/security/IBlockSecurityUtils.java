...
public interface IBlockSecurityUtils {
    ...
    /// Provides access to Mekanism's implementation of [IBlockSecurityUtils].
    ///
    /// @since 10.5.0
    ...
    /// Checks if a player can access the block at the given location; validating that protection is enabled in the config. Additionally, this method also checks to see
    /// if operators bypassing security is enabled in the config and if it is, provides access to the player if they are an operator.
    ///
    /// @param player Player to check access for.
    /// @param level  Level to query.
    /// @param pos    Position in level.
    ///
    /// @return `true` if the player can access the block.
    ///
    ...
    /// @param pos    Position in level.
    ///
    /// @return `true` if the player can access the block at the given location. If the player is `null` this will return `true` if the block's security is
    /// [SecurityMode#PUBLIC].
    ///
    ...
    /// Gets the owner of the block at the given location or `null` if there is no owner or the block doesn't expose an [IOwnerObject].
    ///
    /// @param level Level to query.
    /// @param pos   Position in level.
    ///
    /// @return UUID of the block's owner or `null` if there is no owner.
    ///
    ...
    /// Gets the "effective" security mode for a block at the given location. If it does not expose a [`security object`][ISecurityObject], then the security will be
    /// assumed to be [SecurityMode#PUBLIC] _unless_ an [IOwnerObject] is exposed, in which case the security will be assumed [SecurityMode#PRIVATE] if protection is
    /// enabled.
    ///
    /// When a [`security object`][ISecurityObject] is exposed; this method is _different_ from just querying [ISecurityObject#getSecurityMode()] as this method takes
    /// into account whether protection is disabled in the config and whether the owner of the [ISecurityObject] has their security frequency configured to override the
    /// access level of less restrictive [`security objects`][ISecurityObject].
    ///
    /// @param level Level to query.
    /// @param pos   Position in level.
    ///
    ...
    ///
    /// @implNote If the block doesn't expose a [`security object`][ISecurityObject], then the returned mode is [SecurityMode#PUBLIC]
    ...
    /// @param pos    Position in level.
    ///
    /// @return `true` if the player can access the block at the given location.
    ///
    /// @implNote This method assumes that if the security is [SecurityMode#TRUSTED] and there is a clientside player, then the player can access the [`security
    /// object`][ISecurityObject]. This is done because the list of trusted players is not currently synced to all clients.
    /// @see #canAccess(Player, Level, BlockPos)
