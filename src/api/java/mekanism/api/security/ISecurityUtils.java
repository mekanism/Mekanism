...
public interface ISecurityUtils {
    ...
    /// Provides access to Mekanism's implementation of [ISecurityUtils].
    ///
    /// @since 10.4.0
    ...
    /// @param securityProvider Supplier to get the security capability to check. Can return `null` if there is no security.
    /// @param ownerProvider    Supplier to get the owner capability to check. Can return `null` if there is no owner.
    ///
    ...
    ///
    /// @return `true` if the player can access the given security object.
    ///
    /// @implNote This method assumes that if the security is [SecurityMode#TRUSTED] and there is a clientside player, then the player can access the [`security
    /// object`][ISecurityObject]. This is done because the list of trusted players is not currently synced to all clients.
    ...
    /// @param isClient`true` if this method is being run clientside.
    ///
    /// @return `true` if the player can access the given provider. If the player is `null` this will return `true` if the provider's security is
    /// [SecurityMode#PUBLIC].
    ///
    /// @implNote This method assumes that if the security is [SecurityMode#TRUSTED] and there is a player and `isClient` is `true`, then the player can
    /// access the [`security object`][ISecurityObject]. This is done because the list of trusted players is not currently synced to all clients.
    /// @see #canAccess(Player, Supplier, Supplier)
    /// @see #canAccessObject(Player, ISecurityObject)
    /// @see #canAccessObject(UUID, ISecurityObject, boolean)
    /// @since 10.5.18
    ...
    /// @param isClient`true` if this method is being run clientside.
    ///
    /// @return `true` if the player can access the given security object. If the player is `null` this will return `true` if the object's security is
    /// [SecurityMode#PUBLIC].
    ///
    /// @implNote This method assumes that if the security is [SecurityMode#TRUSTED] and there is a player and `isClient` is `true`, then the player can
    /// access the [`security object`][ISecurityObject]. This is done because the list of trusted players is not currently synced to all clients.
    ...
    ///
    /// If the base is [SecurityMode#PUBLIC] and the override is either [SecurityMode#TRUSTED] or [SecurityMode#PRIVATE].
    ///
    /// If the base is [SecurityMode#TRUSTED] and the override is [SecurityMode#PRIVATE].
    ///
    /// @param base       Base security mode.
    /// @param overridden Override security mode.
    ///
    /// @return `true` if the override security mode is more restrictive than the base security mode.
    ...
    /// Gets the "effective" security mode for a given object. This is _different_ from just querying [ISecurityObject#getSecurityMode()] as this method takes
    /// into account whether protection is disabled in the config and whether the owner of the [ISecurityObject] has their security frequency configured to override
    /// the access level of less restrictive [`security objects`][ISecurityObject].
    ///
