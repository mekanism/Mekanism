...
/// Expose this as a capability on items, entities, or block entities to represent it is an object that have "security". It is **IMPORTANT** to make sure that if you
/// expose this as a capability, you _must_ also expose this object as an [`owner`][IOwnerObject] capability.
///
/// @apiNote The exposed capability should not care about side, and in general will be interacted with via the null side.
///
/// While an [`owner object`][IOwnerObject] _must_ be exposed if this object is exposed, it is not safe to assume that just because an exposed [IOwnerObject] is an
/// instance of an [ISecurityObject] that the object actually has security. The only way to know for certain if it does is by checking if the provider exposes a security
/// capability.
/// @since 10.2.1
public interface ISecurityObject extends IOwnerObject {
    ...
    ///
    /// @apiNote To retrieve the "effective" security mode of this object [ISecurityUtils#getSecurityMode(Supplier, Supplier, boolean)] or
    /// [ISecurityUtils#getEffectiveSecurityMode(ISecurityObject, boolean)] should be used instead.
