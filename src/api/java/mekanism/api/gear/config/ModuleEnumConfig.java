...
public class ModuleEnumConfig<TYPE extends Enum<TYPE> & IHasTextComponent> extends ModuleConfig<TYPE> {
    ...
    private ModuleEnumConfig(Identifier name, TYPE value) {
        this(name, value, List.of(value.getDeclaringClass().getEnumConstants()));
    }

    private ModuleEnumConfig(Identifier name, TYPE value, List<TYPE> enumConstants) {
        super(name);
        this.value = value;
    ...
    @Override
    public StreamCodec<FriendlyByteBuf, ModuleConfig<TYPE>> namedStreamCodec(Identifier name) {
