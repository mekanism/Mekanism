...
///
/// @apiNote Currently Mekanism only has rendering/GUI support for handling [ModuleBooleanConfig], [ModuleColorConfig], and [ModuleEnumConfig]; if more types are needed
/// either open an issue or create a PR implementing support for them.
/// @since 10.6.0
public abstract class ModuleConfig<DATA> {
    ...
    ///
    /// @return `false`, unless overridden to conditionally disable.
    ///
    /// @apiNote When overriding, make sure to also override [#get()], [#with(Object)], [#namedStreamCodec(Identifier)], and use a custom codec and stream codec when
    /// adding the config to the module data.
