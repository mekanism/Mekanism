...
public class MekanismDataGenerator {
    ...
    public static void gatherData(GatherDataEvent event) {
        ...
        addProvider(gen, event.includeClient(), output -> new PrideRobitTextureProvider(output, existingFileHelper));
        addProvider(gen, event.includeClient(), output -> new MekanismSoundProvider(output, existingFileHelper));
        addProvider(gen, event.includeClient(), output -> new MekanismSpriteSourceProvider(output, existingFileHelper));
        //Let the blockstate provider see models generated by the item model provider
        MekanismItemModelProvider itemModelProvider = new MekanismItemModelProvider(gen.getPackOutput(), existingFileHelper);
        gen.addProvider(event.includeClient(), itemModelProvider);
        addProvider(gen, event.includeClient(), output -> new MekanismBlockStateProvider(output, itemModelProvider.existingFileHelper));
        ...
        //addProvider(gen, event.includeServer(), output -> new MekanismCustomConversions(gen));
        ...
        addProvider(gen, true, output -> new PersistingDisabledProvidersProvider(output, recipeProvider.getDisabledCompats()));
    }
