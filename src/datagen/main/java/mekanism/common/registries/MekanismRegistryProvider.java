...
public class MekanismRegistryProvider extends BaseRegistryProvider {
    ...
    public static DatapackBuiltinEntriesProvider forReloadableLayer(PackOutput output, CompletableFuture<HolderLookup.Provider> worldRegistries,
        ...
        return forReloadableLayer(output, worldRegistries, reloadableRegistries, Mekanism.MODID, new RegistrySetBuilder()
              ...
              .add(Registries.CONTEXT_INT_PROVIDER, context -> {
                  ...
                  context.register(MekanismContextIntProviders.COOKING_TIME_CHARCOAL_BLOCK, cooking(predicates, normalBurnTime, fastBurnTime, 16_000));
                  int bioFuelBurnTime = 5 * SharedConstants.TICKS_PER_SECOND;
                  context.register(MekanismContextIntProviders.COOKING_TIME_BIO_FUEL, cooking(predicates, normalBurnTime, fastBurnTime, bioFuelBurnTime));
    ...
    private static ResizableOreFeature configureOreFeature(OreVeinType oreVeinType, boolean retrogen) {
        OreVeinConfig oreVeinConfig = MekanismConfig.world.getVeinConfig(oreVeinType);
        List<BlockReplacement> targetStates = ORE_STONE_TARGETS.computeIfAbsent(oreVeinType.type(), oreType -> {
            OreBlockType oreBlockType = MekanismBlocks.ORES.get(oreType);
            return List.of(
                  BlockReplacement.replace(STONE_ORE_REPLACEABLES, oreBlockType.stone().defaultState()),
                  BlockReplacement.replace(DEEPSLATE_ORE_REPLACEABLES, oreBlockType.deepslate().defaultState())
            );
        });
